...
export default function ReservationForm({
  ...
  useEffect(() => {
    ...
    const initDates = async () => {
      // Sync with server clock first so a wrong device clock can't shift dates
  ...
  const handleFastAgentComplete = (
    ...
    const dashboardUrl = userRole === "admin" ? "/dashboard" : "/customerDashboard";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(age) || age < minDriverAge || age > 80) {
      showToast.error(`Driver age must be between ${minDriverAge} and 80 years`);
      ...
      >
        ...
        <div>
          ...
          <div className="relative">
            ...
            {showDateRange && (
              <div className="absolute -mt-20 left-0 min-h-44 max-h-120 overflow-y-auto -top-44 z-50 bg-slate-800 backdrop-blur-xl border border-white/20 rounded-lg p-4 w-70 sm:w-75  ">
                <DateRange
                  className="w-full"
                  ranges={dateRange}
                  onChange={(item) => {
